...
### selecionar apenas a primeira linha
...
cores_plot02 <- c("#CC5A71", "#536265", "#536265", "#536265", "#536265","#536265",
                  ...
                  "#536265")                                # vamos destacar apenas o mês out/2018 no gráfico.
...
plot02 <- meses_mais_qte|> 
  ...
  theme_imdb()                        # personaliza as configurações do tema definidas na função theme_imdb().
...
mes_maior_lancamento$mes <- NULL                                                        # Exclusão de coluna. 
mes_maior_lancamento$qte <- NULL                                                        # Exclusão de coluna.

# formatar a variável data_lancamento para a tabela 02

mes_maior_lancamento <- mes_maior_lancamento |> 
  mutate(data_lancamento = format(data_lancamento, "%d-%m-%Y"))         # altera o formato da data para D-M-Y.

# Tabela com os 5 filmes  de outubro de 2018 com melhores rankings
...
::: {align="justify" style="text-indent: 3em;"}
O filme "**`r imdb_nasce_uma_estrela$titulo`**" foi  lançado  no Brasil como "**Nasce uma Estrela**". Pertence aos gêneros **`r imdb_nasce_uma_estrela$genero`**. Sua *premiere* ocorreu em `r format(imdb_nasce_uma_estrela$data_lancamento, "%d-%m-%Y")`,  com duração de `r imdb_nasce_uma_estrela$duracao` minutos sob a direção de `r imdb_nasce_uma_estrela$direcao`. Roteiro de `r imdb_nasce_uma_estrela$roteiro`.  
...
::: {align="justify" style="text-indent: 3em;"}
O filme "**`r imdb_searching$titulo`**" foi  lançado  no Brasil como "**Buscando**". Pertence aos gêneros **`r imdb_searching$genero`**. Sua *premiere* ocorreu em `r format(imdb_searching$data_lancamento, "%d-%m-%Y")`,  com duração de `r imdb_searching$duracao` minutos sob a direção de `r imdb_searching$direcao`. Roteiro de `r imdb_searching$roteiro`.  
...
::: {align="justify" style="text-indent: 3em;"}
O filme "**`r imdb_firstman$titulo`**" foi  lançado  no Brasil como "**O Primeiro Homem**". Pertence aos gêneros **`r imdb_firstman$genero`**. Sua *premiere* ocorreu em `r format(imdb_firstman$data_lancamento, "%d-%m-%Y")`,  com duração de `r imdb_firstman$duracao` minutos sob a direção de `r imdb_firstman$direcao`. Roteiro de `r imdb_firstman$roteiro`.  
...
::: {align="justify" style="text-indent: 3em;"}
O filme "**`r imdb_soldado$titulo`**" foi  lançado  no Brasil como "**Sicario 2 - Soldado**". Pertence aos gêneros **`r imdb_soldado$genero`**. Sua *premiere* ocorreu em `r format(imdb_soldado$data_lancamento, "%d-%m-%Y")`,  com duração de `r imdb_soldado$duracao` minutos sob a direção de `r imdb_soldado$direcao`. Roteiro de `r imdb_soldado$roteiro`.  
...
::: {align="justify" style="text-indent: 3em;"}
O filme "**`r imdb_royale$titulo`**" foi  lançado  no Brasil como "**Maus Momentos no Hotel Royale**". Pertence aos gêneros **`r imdb_royale$genero`**. Sua *premiere* ocorreu em `r format(imdb_royale$data_lancamento, "%d-%m-%Y")`,  com duração de `r imdb_royale$duracao` minutos sob a direção de `r imdb_royale$direcao`. Roteiro de `r imdb_royale$roteiro`.  
...
tab03 <- mes_maior_lancamento_tab03 |> 
  kbl(align = "l",                                            # alinhamento dos textos nas colunas a esquerda.
      col.names = c("Mês",                                                        
                    "Quantidade de Filmes"
                    ),
      full_width = TRUE,
...
plot03<- imdb_novo |> 
  ...
  theme_classic()+                          
  scale_y_continuous(breaks = seq(0,                                              # formatar escala do eixo y.
                                  8000,
                                  1000
                                  ),                                               
                     limits = c(0,
  ...
  scale_x_continuous(breaks = seq(0,                                              # formatar escala do eixo x.
                                  31,
                                  1
                                  )
                     ) +                                                         
...
plot04 <- top_22_paises |> 
  ggplot(aes(y = fct_reorder(pais,      # reordena  a sequência dos países em ordem decrescente da quantidade.
                             qtde, 
                             .desc = FALSE
                             ),
             x = qtde, 
  ...
  geom_curve(aes(y = 18,                                                       # formata  curva para anotação.
                 x = 8000,  
                 xend = 16000,     
                 yend = 16
                 ),
  ...
  geom_curve(aes(y = 1,                                                         # formata curva para anotação.
                 x = 2800,  
                 xend = 11000,     
                 yend = 2
                 ),
...
lucros_genero <- imdb_filmes_dolares_genero |>
  ...
  slice_head(n=10) |> 
  mutate(lucro_total = paste("US$",          # formata o valor obtido para o formato com US$ para a tabela 08.
...
::: {align="justify" style="text-indent: 3em;"}
"**`r imdb_avatar$titulo`**" pertence aos gêneros **`r imdb_avatar$genero`**. Sua *premiere* ocorreu em `r format(imdb_avatar$data_lancamento, "%d-%m-%Y")`,  com duração de `r imdb_avatar$duracao` minutos sob a direção `r imdb_avatar$direcao`.
...
::: {align="justify" style="text-indent: 3em;"}
O lucro médio dos `r nrow(filmes_cameron)` filmes listados na base Imdb foi de `r lucro_medio_filmes`. **Avatar** com um ganho de `r filmes_cameron$lucro[3]` e Titanic com um ganho de `r filmes_cameron$lucro[4]` lideram a lista dos mais lucrativos.
...
imdb_tab14 <- imdb_novo_ordenado |> 
  ...
  mutate(data_lancamento_dia = wday(data_lancamento,
         ...
         data_lancamento = format(data_lancamento, "%d-%m-%Y")                    # formata a data para D-M-Y.
         ) |>
...
::: {align="justify" style="text-indent: 3em;"}
A estreia nas salas de cinemas dos Estados Unidos ocorreu em `r format(imdb_avatar$data_lancamento, "%d-%m-%Y")`, em uma sexta-feira. Neste dia, tinha `r as.duration(year("2010-01-15") - year("1989-06-07"))` anos de idade. 
...
tab15 <- imdb_novo_ordenado |> 
  ...
  mutate(data_lancamento = format(data_lancamento, "%d-%m-%Y")) |>                  # formata data para D-M-Y.
  select(titulo,
         data_lancamento,
         genero,
         nota_imdb,
